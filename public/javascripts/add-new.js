...
function addField(){
	...
	$('.form-control').each(function(){
		if($(this).attr('type') == 'text' || $(this).attr('name') == 'select_experience'){
			if($(this).val() == ''){
				EmptyFields = true;
			}
		}
...
var clickID = parseInt($(this).parent().parent().parent('div').attr('id').replace('div_',''));
...
$newClone.children().children().children().children("input").eq(0).attr("id",'company'+newID).val('');

//Borro el valor del segundo campo input(este caso es el campo de cantidad)
$newClone.children().children().children().children("input").eq(1).attr("id",'job'+newID).val('');

//Nuevo campo
$newClone.children().children().children("select").eq(0).attr("id",'experience'+newID).val('');

//Asigno nuevo id al boton
$newClone.children().children().children("img").eq(0).attr("id",newID);

//Inserto el div clonado y modificado despues del div original
$newClone.insertAfter($('#div_'+clickID));

//Cambio el signo "+" por el signo "-" y le quito el evento addfield
$("#"+clickID).attr('src', '/assets/images/orientation/ic_trash2.png').unbind("click",addField);
